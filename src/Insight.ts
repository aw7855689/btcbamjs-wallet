...
const INSIGHT_BASEURLS: { [key: string]: string } = {
  [NetworkNames.MAINNET]: "https://insight-api.sbercoin.com",
...
export class Insight {
  ...
  public async sendRawTx(rawtx: string): Promise<Insight.ISendRawTxResult> {
    const res = await axios.post('https://explorer.sbercoin.com/api/tx/send', {
      rawtx,
    }).then(response => {
  ...
  public async estimateFee(nblocks: number = 6): Promise<any> {
    const res = await axios.get('https://explorer.sbercoin.com/api/info').then(function (response) {
